...
myLR_age = MyLR(thetas=[[1000.0], [-1.0]], alpha=2.5e-4, max_iter=25000)
...
Y = np.array(data[["Sell_price"]])
myLR_age = MyLR(thetas=[[1], [1]], alpha=2.5e-6, max_iter=25000)
myLR_age.fit_(X[:, 0].reshape(-1, 1), Y)
y_pred = myLR_age.predict_(X[:, 0].reshape(-1, 1))
...
Y = np.array(data[["Sell_price"]])
myLR_distance = MyLR(thetas=[[732.0], [-3.0]], alpha=2.5e-6, max_iter=50000)
...
Y = np.array(data[["Sell_price"]])
my_lreg = MyLR(thetas=[[1], [1], [1], [1]], alpha=9e-5, max_iter=600000)
my_lreg.fit_(X, Y)
